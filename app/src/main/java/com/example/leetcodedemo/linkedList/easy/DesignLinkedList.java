...
public class DesignLinkedList {
    class MyLinkedList {
        ...
        private ArrayList<ListNode> linkList;
        ...
        public void addAtTail(int val) {
            ListNode node = new ListNode(val);
            if (linkList.size() > 0)
                linkList.get(linkList.size() - 1).next = node;
            linkList.add(node);
        }
        ...
        public void addAtIndex(int index, int val) {
            if (index <= linkList.size()) {
                if (index <= 0)
                    ...
                    ListNode node = new ListNode(val);
                    node.next = linkList.get(index);
                    linkList.get(index - 1).next = node;
                    linkList.add(index, node);
                }
            }
        }
        ...
        public void deleteAtIndex(int index) {
            if (linkList.size() > 0) {
                if (index == 0)
                    ...
                    linkList.get(index - 1).next = null;
                }
                else if (index > 0 && index < linkList.size()  -1) {
                    linkList.get(index - 1).next = linkList.get(index + 1);
                    linkList.remove(index);
                }
            }
        }
